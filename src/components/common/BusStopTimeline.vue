...
const busProgress = computed(() => {
  if (stops.value.length < 2) return { currentIdx: 0, fraction: 0, isBeforeStart: true, isAfterEnd: false }
  ...
  if (nowSec < departureMin) {
    return { currentIdx: 0, fraction: 0, isBeforeStart: true, isAfterEnd: false }
  }
...
const currentStopName = computed(() => {
  const { currentIdx, isBeforeStart, isAfterEnd } = busProgress.value
  if (isBeforeStart) return stops.value[0]?.stopName ?? ''
  if (isAfterEnd) return stops.value[stops.value.length - 1]?.stopName ?? ''
  return stops.value[currentIdx]?.stopName ?? ''
})
...
<template>
  <div class="timeline-root">
    <!-- 公交车进度条指示器 -->
    <div class="bus-indicator" :style="{ top: busTopPercent + '%' }">
      <img
    ...
    <div class="stop-list">
      <div
        ...
        class="stop-row"
        :class="{
          'is-start': stop.isDepartureStop,
          'is-end': stop.isReturnStop,
          'is-passed': idx <= busProgress.currentIdx && !busProgress.isBeforeStart && !busProgress.isAfterEnd,
        }"
      >
        <!-- 圆点 -->
...
.stop-row.is-passed .stop-name,
.stop-row.is-passed .stop-time {
  opacity: 0.45;
}
